...
(deftest transformer-op-test-cljs
  (is (= 3 (apply (-> t/transformer (assoc :op +)) [1 2])))
  (is (= -4 (apply (-> t/transformer (assoc :op -)) [1 2 3])))
  (is (= 24 (apply (-> t/transformer (assoc :op *)) [1 2 3 4])))
  (is (= 0.041666666666666664 (apply (-> t/transformer (assoc :op /)) [1 2 3 4])))
  (is (= '(:k1 :k2) (apply (-> t/transformer (assoc :op keys)) [{:k1 1 :k2 2}])))) ; Ends with 4 )

(deftest transformer-in-test-cljs
  ...
  (is (= [3 6]
         (let [args [1 2]]
           (-> (-> t/transformer
                   (assoc :env-op (fn [env] env)) ; CLJS version
...
(deftest transformer-out-test-cljs
